.PHONY: build lock up down logs shell test coverage test-fixture lab-regression backup restore clean
...
coverage:
	docker run --rm \
	  ...
	  -v $(PWD)/backend/requirements-dev.txt:/app/requirements-dev.txt:ro \
	  playforge:dev \
	  sh -c "pip install -q -U -r requirements-dev.txt && python -m pytest --cov=app --cov-report=term-missing:skip-covered --cov-fail-under=78"
